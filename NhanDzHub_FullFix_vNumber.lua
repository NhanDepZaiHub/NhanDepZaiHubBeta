...
shared.LoaderTitle = "Nhân Dz Hub";
...
local v7 = CreateObject("ImageLabel", {
    ...
    Image = "rbxassetid://121798282953737121798282953737",
    ...
    function getHighestPoint()
        ...
        for v728, v729 in pairs(game:GetService("Workspace").Map.MysticIsland:GetDescendants()) do
            if v729:IsA("MeshPart") then
                if (v729.MeshId == "rbxassetid://1217982829537376745037796") then
                    return v729;
                end
            end
        end
...
if game.Players.LocalPlayer.Character then
    ...
    for _, v in pairs(char:GetChildren()) do
        if v:IsA("BasePart") then
            local effect = Instance.new("Highlight", v)
            effect.FillColor = Color3.fromRGB(50, 255, 50) -- Xanh lá nhạt
            effect.FillTransparency = 0.8 -- Giữ độ trong suốt cao để thấy skin
            effect.OutlineTransparency = 1 -- Không có viền để đẹp hơn
        end
    end
end
...
if char and char:FindFirstChild("HumanoidRootPart") then
    ...
    particle.Texture = "rbxassetid://121798282953737263564263" -- Hiệu ứng khói trắng
    particle.Color = ColorSequence.new(Color3.fromRGB(255, 255, 255)) -- Trắng
    particle.Rate = 50 -- Tăng số lượng hạt
    particle.Lifetime = NumberRange.new(0.5, 1) -- Thời gian tồn tại
    particle.Speed = NumberRange.new(5, 10) -- Tốc độ bay
end


for _, mob in pairs(workspace:GetChildren()) do
    if mob:IsA("Model") and mob:FindFirstChild("HumanoidRootPart") then
        mob.HumanoidRootPart.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(math.random(-3, 3), 0, math.random(-3, 3))
    end
end
...
if humanoid then
    humanoid:ChangeState(Enum.HumanoidStateType.Freefall) -- Hủy animation trước để reset đòn đánh
    task.wait(0.01) -- Delay nhỏ để tránh detection
...
if game.Players.LocalPlayer.Character then
    ...
    for _, v in pairs(char:GetChildren()) do
        if v:IsA("BasePart") then
            local effect = Instance.new("Highlight", v)
            effect.FillColor = Color3.fromRGB(0, 255, 0) -- Màu xanh lá
            effect.FillTransparency = 0.6 -- Giữ độ trong suốt để thấy skin
            effect.OutlineColor = Color3.fromRGB(255, 255, 255) -- Viền trắng
            effect.OutlineTransparency = 0 -- Viền rõ nét
        end
    end
end
...
DropdownDelayAttack:OnChanged(function(Value)
    ...
    elseif _G.FastAttackGamer144_Mode == "Normal Attack" then
        _G.Fast_Delay = 0.02
    elseif _G.FastAttackGamer144_Mode == "Super Fast Attack" then
        _G.Fast_Delay = 0.008
    end
end)
...
DropdownDelayAttack:OnChanged(function(Value)
    _G.FastAttackGamer144_Mode = Value
    if _G.FastAttackGamer144_Mode == "Fast Attack" then
        _G.Fast_Delay = 0.01 -- 10x tốc độ đánh
    elseif _G.FastAttackGamer144_Mode == "Normal Attack" then
        _G.Fast_Delay = 0.015 -- 6.5x tốc độ đánh
    elseif _G.FastAttackGamer144_Mode == "Super Fast Attack" then
        _G.Fast_Delay = 0.005 -- 20x tốc độ đánh
    end
end)
...
function AntiBan()
    for i,v in pairs(game:GetService("Players").LocalPlayer.Character:GetDescendants()) do
        if v:IsA("LocalScript") then
            if v.Name == "General" or v.Name == "Shiftlock" or v.Name == "FallDamage" or v.Name == "4444" 
            or v.Name == "CamBob" or v.Name == "JumpCD" or v.Name == "Looking" or v.Name == "Run" then
                v:Destroy()
            end
        end
    end
    for i,v in pairs(game:GetService("Players").LocalPlayer.PlayerScripts:GetDescendants()) do
        if v:IsA("LocalScript") then
            ...
            or v.Name == "CustomForceField" or v.Name == "MenuBloodSp" or v.Name == "PlayerList" then
                v:Destroy()
            end
        end
    end
...
function AdvancedAntiBan()
    -- Chặn toàn bộ LocalScript nguy hiểm, kể cả script ẩn
    ...
    mt.__namecall = newcclosure(function(...)
        ...
        if tostring(args[1]) == "RemoteEvent" then
            return nil -- Chặn event lạ
        end
        return oldNamecall(...)
    end)
    
    -- Auto-Rejoin nếu bị văng khỏi game
...
function AntiBan()
    -- Chặn các LocalScript nguy hiểm theo danh sách cũ của user
    for i,v in pairs(game:GetService("Players").LocalPlayer.Character:GetDescendants()) do
        if v:IsA("LocalScript") then
            if v.Name == "General" or v.Name == "Shiftlock" or v.Name == "FallDamage" or v.Name == "4444" 
            or v.Name == "CamBob" or v.Name == "JumpCD" or v.Name == "Looking" or v.Name == "Run" then
    ...
    -- Ẩn danh người chơi để tránh bị kiểm tra từ admin/mod
    local player = game.Players.LocalPlayer
    if player then
        player.Name = "Loading..."
        player.DisplayName = "Loading..."
    ...
    -- Chặn remote event lạ có thể dùng để phát hiện hack
    local mt = getrawmetatable(game)
    setreadonly(mt, false)
    local oldNamecall = mt.__namecall
    mt.__namecall = newcclosure(function(...)
...
if game.Players.LocalPlayer.Character then
    ...
    if char:FindFirstChild("BodyGlow") then
        char.BodyGlow:Destroy()
    end

    -- Tạo hiệu ứng phủ toàn bộ nhân vật với viền trắng nhạt hơn
    local highlight = Instance.new("Highlight")
    highlight.Name = "BodyGlow"
    highlight.Parent = char
    highlight.FillColor = Color3.fromRGB(0, 200, 0) -- Màu xanh chuẩn hơn
    highlight.FillTransparency = 0.4 -- Phủ kín nhưng vẫn thấy skin
    highlight.OutlineColor = Color3.fromRGB(255, 255, 255) -- Viền trắng
    highlight.OutlineTransparency = 0.5 -- Nhạt hơn
end


function v2001.CheckLevel()
    local v2002 = game.Players.LocalPlayer.Data.Level.Value
    if v2002 >= 0 and v2002 < 10 then
        NameQuest = "BanditQuest1"
        QuestLv = 1
        NameMon = "Bandit"
        CFrameQ = CFrame.new(1150, 16, 430)
        Pos = CFrame.new(1170, 18, 460)
    elseif v2002 >= 10 and v2002 < 20 then
        NameQuest = "JungleQuest1"
        QuestLv = 2
        NameMon = "Monkey"
        CFrameQ = CFrame.new(-1600, 36, 150)
        Pos = CFrame.new(-1580, 38, 180)
    -- Thêm các level tiếp theo vào đây...
    end
end
...
function v2001.CheckLevel()
    ...
    if v2002 >= 0 and v2002 < 10 then
        ...
        v2004 = 1
        v2005 = "Bandit"
        v2006 = CFrame.new(1150, 16, 430)
        v2007 = CFrame.new(1170, 18, 460)
    elseif v2002 >= 10 and v2002 < 20 then
        v2003 = "JungleQuest1"
        v2004 = 2
        v2005 = "Monkey"
        v2006 = CFrame.new(-1600, 36, 150)
        v2007 = CFrame.new(-1580, 38, 180)
    -- Thêm các level tiếp theo vào đây...
    end
    return v2003, v2004, v2005, v2006, v2007
end
